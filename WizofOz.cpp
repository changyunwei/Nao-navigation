...
WoZControlClient::WoZControlClient(QWidget *parent, Qt::WFlags flags)
...
{
    ui.leVX->setText( tr("%1").arg(x) );
    ui.leVY->setText( tr("%1").arg(y) );
    ui.leVTheta->setText( tr("%1").arg(theta) );
    ui.leVF->setText( tr("%1").arg(f) );
...
void WoZControlClient::pbOnClickTurnRight(bool b)
{
    ...
    {
	    robotState->theta = -ui.dsbVTheta->value();
        ui.pbTurnLeft->setChecked(false);
    }
    else
    {
        robotState->theta = 0;
...
void WoZControlClient::pbOnClickStop()
{
    ...
    ui.pbForwarding->setChecked(false);
    ui.pbBackwarding->setChecked(false);
    ui.pbLeftsiding->setChecked(false);
    ui.pbRightsiding->setChecked(false);
}
...
void WoZControlClient::pbOnClickForwarding(bool b)
{
    if ( b )
    {
        robotState->vx = ui.dsbVX->value();
        robotState->frequency = ui.dsbVF->value();
        ui.pbBackwarding->setChecked(false);
    }
    else 
    {
...
void WoZControlClient::pbOnClickRstHeadPitch()
{
    ...
    // ui display
    ui.leHeadPitch->setText(tr("%1").arg(0));
    ui.vsldHeadPitch->setSliderPosition(0);
}
...
void WoZControlClient::leOnTextChangedHeadPitch(QString sHP)
{
    ...
    // ui display
    ui.leHeadPitch->setText(tr("%1").arg(finalFHP));
    ui.vsldHeadPitch->setSliderPosition(finalFHP*1000.0);
}
...
void WoZControlClient::pbOnClickRstHeadYaw()
{
    ...
    // ui display
    ui.leHeadYaw->setText(tr("%1").arg(0));
    ui.hsldHeadYaw->setSliderPosition(0);
}
...
void WoZControlClient::leOnTextChangedHeadYaw(QString sHY)
{
    ...
    // ui display
    ui.leHeadYaw->setText(tr("%1").arg(finalFHY));
    ui.hsldHeadYaw->setSliderPosition(finalFHY*1000.0);
}
